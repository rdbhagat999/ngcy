...
@Component({
  ...
  template: `
    <section
      class="home grid grid-cols-1 gap-4">
      <article>
        <div class="overflow-hidden bg-white shadow sm:rounded-lg">
          ...
          <div class="px-4 pb-5 sm:px-6">
            ...
            <div class="space-x-4 my-2">
              <button class="px-4 py-2 bg-blue-500 text-white" type="button" (click)="increment()">+</button>
              <button class="px-4 py-2 bg-blue-500 text-white" type="button" (click)="decrement()">-</button>
            </div>
            <p>Is Signal value EVEN?: {{isEvenComputedSignal() == 0 ? 'even' : 'odd'}}</p>
          </div>
        </div>
      </article>
    </section>
